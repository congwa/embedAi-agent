...
export default function SingleModeAgentsPage() {
  ...
  return (
    <div className="space-y-6">
        ...
        <Card className="border-emerald-200 bg-emerald-50/50 dark:border-emerald-800 dark:bg-emerald-950/30">
          <CardContent className="flex items-center justify-between p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="font-medium">当前激活：{currentAgent.name}</div>
                <div className="text-sm text-zinc-500">所有用户请求将由此 Agent 处理</div>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      {filteredAgents.length === 0 ? (
        ...
        <AgentList agents={filteredAgents} basePath="/admin/single/agents" />
      )}
    </div>
  );
}
